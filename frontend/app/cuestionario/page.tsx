                          ...
                          <td className="px-3 py-3">
                            <div className="flex items-center gap-1">
                              {lead.telefono_whatsapp && (() => {
                                ...
                                const producto = lead.productos_recomendados?.[0] ?? "";
                                const productoLabel = NOMBRES_PRODUCTOS[producto] ?? producto;
                                const textoWA = `Hola ${nombre}, soy Manuel de Nationale-Nederlanden. Me llega que has rellenado nuestro formulario de asesoramiento${productoLabel ? ` — veo que te puede interesar ${productoLabel}` : ""}. ¿Cuándo te va bien que hablemos 10 minutos?`;
                                return (
                                  <a
                                    href={`https://wa.me/${lead.telefono_whatsapp.replace(/[^0-9]/g, "")}?text=${encodeURIComponent(textoWA)}`}
                                    target="_blank"
                                    rel="noopener noreferrer"
